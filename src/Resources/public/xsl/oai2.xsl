...
>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<p>If you want to use the XSL file on your own OAI interface you may but due to the way XSLT works you must install the XSL file on the same server as the OAI script, you can't just link to this copy.</p>
	...
	<xsl:template name="quicklinks">
		<ul class="quicklinks">
			...
			<li>
				<a href="?verb=ListRecords&amp;metadataPrefix=oai_dc">ListRecords (oai_dc)</a> | </li>
			<li>
				<a href="?verb=ListRecords&amp;metadataPrefix=mets">ListRecords (mets)</a> | </li>
			<li>
				<a href="?verb=ListIdentifiers&amp;metadataPrefix=oai_dc">ListIdentifiers (oai_dc)</a> | </li>
			<li>
				<a href="?verb=ListIdentifiers&amp;metadataPrefix=mets">ListIdentifiers (mets)</a> | </li>
			<li>
				<a href="?verb=ListRecords&amp;metadataPrefix=epicur">ListRecords (epicur)</a> | </li>
			<li>
		...
		<table class="values">
			<tr>
				...
				<td class="value">
					...
					<xsl:text> </xsl:text>
					<a class="link" href="https://dfg-viewer.de/show/?tx_dlf[id]=https://gdz.sub.uni-goettingen.de/mets/{substring(oai:identifier,31)}.mets.xml">DFG-Viewer</a>
					<xsl:text> </xsl:text>
  ...
  &#160;
	...
	<xsl:template match="oai:resumptionToken">
		...
		<table class="values">
			<tr>
				...
				<td class="value">
					<xsl:value-of select="."/> (List size: <xsl:value-of select="@completeListSize"/> / Cursor:  <xsl:value-of select="@cursor"/>)
					<xsl:text> </xsl:text>
    ...
    &lt;
			...
			<xsl:apply-templates select="@*" mode='xmlMarkup'/>&gt;
			<xsl:apply-templates select="node()" mode='xmlMarkup' />&lt;/<span class="xmlTagName">
				<xsl:value-of select='name(.)' />
			</span>&gt;
		</div>
	</xsl:template>
